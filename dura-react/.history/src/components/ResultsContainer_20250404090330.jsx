...
const ResultsContainer = ({ results, onSelectCompany }) => {
  ...
  const renderCompanies = (companyList, type) => {
    if (viewMode === 'grid') {
      return (
        <CompaniesGrid key={`${type}-grid`} layout>
          {companyList.map((company) => (
            <CompanyCard 
              key={company.id || company.name} 
              company={company}
              onClick={() => onSelectCompany(company)}
              layoutId={`company-${company.id || company.name}`}
            />
          ))}
        </CompaniesGrid>
      );
      ...
      return (
        <CompaniesList key={`${type}-list`} layout>
          {companyList.map((company) => (
            <CompanyListItem 
              key={company.id || company.name}
              onClick={() => onSelectCompany(company)}
              ...
              <motion.div layout>
                <ScoreAndStatus>
                  <Score score={company.partnership_score}>
                    {Math.floor(company.partnership_score)}
                  </Score>
