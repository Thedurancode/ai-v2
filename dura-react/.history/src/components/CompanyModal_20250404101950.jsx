...
const CompanyModal = ({ isOpen, onClose, company }) => {
  ...
  const handleAction = async (action, options = {}) => {
    ...
    if (action === 'perplexity') {
      ...
      try {
        ...
        if (openAIData && typeof openAIData === 'string' && openAIData.length > 100) {
          console.log("OpenAI research successful, data length:", openAIData.length);
          
          // Format the OpenAI response for display
          const sections = formatOpenAIResponse(openAIData);
          ...
          console.error("OpenAI returned invalid or too short research data", openAIData);
  ...
  useEffect(() => {
    if (isOpen && company) {
      setIsLoading(true);
      // Reset Perplexity section visibility when modal is opened
      setShowPerplexitySection(false);
      // Also clear any existing perplexity data to ensure it doesn't show up
      ...
      {isOpen && (
          ...
          >
            ...
            <ModalBody>
              ...
              {showPerplexitySection && (isPerplexityLoading || perplexityData) && (
                <ModalSection
                  variants={sectionVariants}
