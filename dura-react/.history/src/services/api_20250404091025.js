...
export const getDeepSeekResearch = async (company) => {
  console.log("=========== DEEPSEEK DEBUG START ===========");
  try {
    console.log("Attempting DeepSeek research with model: deepseek-chat");
    ...
    console.log("API key first 5 chars:", DEEPSEEK_API_KEY.substring(0, 5));
    
    // Using the exact format that worked in the curl test
    const payload = {
      model: "deepseek-chat", // Use the model that worked in curl test
      messages: [
        {
          role: "user",
          content: query
    ...
    console.error("DeepSeek research failed:", error.message);
    console.log("Falling back to Perplexity");
    console.log("=========== DEEPSEEK DEBUG END ===========");
    
    // Fall back to Perplexity which we know works
    try {
      const perplexityData = await getPerplexityResearch(company);
      // Save the research data from Perplexity
      try {
        await saveCompanyResearch(company.name, perplexityData, 'perplexity');
        console.log(`Successfully saved Perplexity research data for ${company.name}`);
      } catch (saveError) {
        console.error("Failed to save Perplexity research data:", saveError);
      }
